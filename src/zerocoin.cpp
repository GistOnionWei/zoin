...
bool CheckDevFundInputs(const CTransaction &tx, CValidationState &state, int nHeight, bool fTestNet) {
    ...
    /* Check for Libernode payment in block */
    if(nHeight >= Params().GetConsensus().nLibernodePaymentsStartBlock){

        int total_payment_tx = 0;
        CAmount libernodePayment = GetLibernodePayment(nHeight, 0);

        BOOST_FOREACH(const CTxOut &output, tx.vout) {
            if (libernodePayment == output.nValue) {
                total_payment_tx = total_payment_tx + 1;
            }
        }
        // no more than 1 output for payment
        if (total_payment_tx > 1) {
            return state.DoS(100, false, REJECT_INVALID_LIBERNODE_PAYMENT,
                             "CTransaction::CheckTransaction() : invalid libernode payment");
        }
    }
	return true;
}
	...
	if(tx.IsZerocoinSpend()) {
		...
		BOOST_FOREACH(const CTxOut &txout, tx.vout)
		{
			if (!isVerifyDB) {
                ...
                case libzerocoin::ZQ_PEDERSEN*COIN:
                case libzerocoin::ZQ_WILLIAMSON*COIN:
                    if((nHeight >= ZC_V1_5_STARTING_BLOCK + 576)){
