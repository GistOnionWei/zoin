...
bool CLibernodeBroadcast::CheckOutpoint(int &nDos) {
...

    {
        ...
        if (coins.vout[vin.prevout.n].nValue != LIBERNODE_COIN_REQUIRED * COIN) {
            LogPrint("libernode", "CLibernodeBroadcast::CheckOutpoint -- Libernode UTXO should have 25000 LBR, libernode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 25000 LBR tx got nLibernodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex *pMNIndex = (*mi).second; // block for 25000 LBR tx -> 1 confirmation
